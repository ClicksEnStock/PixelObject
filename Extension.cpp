    ...
    : rdPtr(_rdPtr), rhPtr(_rdPtr->rHo.hoAdRunHeader), Runtime(_rdPtr)
{
    ...
    */
    ...
    //LinkAction(0, SetPixelOnScreen);
    //LinkAction(1, SecondActionExample);

    //LinkCondition(0, IsPixel);

    LinkExpression(0, GetRed);
    LinkExpression(1, GetGreen);
	LinkExpression(2, GetBlue);
	LinkExpression(3, GetColor);
...
Extension::~Extension()
{
    ...
    */
	if ( rdPtr->pExtension->renderSurf != NULL )
	{
		delete rdPtr->pExtension->renderSurf;
		rdPtr->pExtension->renderSurf = NULL;
	}
}
